...
namespace BattleShip {
    public partial class Game : Window {
        ...
        public Game(int UserId) {
            ...
            for (int i = 1; i < 11; i++) {
                TextBlock tb = new() { Text = $"{Chars[i - 1]}", Margin = new Thickness(left: marginsForChars[i - 1], 0, 0, 0) }; CharsEnemy.Children.Add(tb);
                TextBlock tb2 = new() { Text = $"{Chars[i - 1]}", Margin = new Thickness(left: marginsForChars[i - 1], 0, 0, 0) }; CharsOur.Children.Add(tb2);

                TextBlock tb3 = new() { Text = $"{i}", Margin = new Thickness(0, top: -2, 0, 0), HorizontalAlignment = HorizontalAlignment.Right }; NumEnemy.Children.Add(tb3);
                TextBlock tb4 = new() { Text = $"{i}", Margin = new Thickness(0, top: -2, 0, 0), HorizontalAlignment = HorizontalAlignment.Right }; NumOur.Children.Add(tb4);
            }            
        }        
